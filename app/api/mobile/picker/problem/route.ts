import {
  getMobilePermissionAccountContext,
  mobileError,
  readMobileJsonBody
} from "@/lib/mobile-api";

export async function POST(request: Request) {
  const body = await readMobileJsonBody(request);

  if (!body.ok) {
    return body.response;
  }

  const context = await getMobilePermissionAccountContext(request, "canReportProblem", body.data.accountId);

  if (!context.ok) {
    return context.response;
  }

  void context;
  return mobileError("legacy_picker_retired", "SKU-group problem reporting was retired. Resolve an exact Pick task and report its problem through Work Details.", 410);
}
