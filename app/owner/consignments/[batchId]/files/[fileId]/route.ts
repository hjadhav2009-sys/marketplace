...
export async function GET(_request: Request, context: { params: Promise<{ batchId: string; fileId: string }> }) {
 ...
 try { const stream=await openConsignmentReadStream(file.managedRelativePath); const safe=file.originalFileName.replace(/["\r\n]/g,"_"); return new Response(stream as never,{headers:{"Content-Type":"application/octet-stream","Content-Disposition":`attachment; filename="${safe}"`,"Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff"}}); } catch { return Response.json({error:"File unavailable."},{status:404}); }
}
