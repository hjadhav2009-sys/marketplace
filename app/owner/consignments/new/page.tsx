...
export default async function NewConsignmentPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <AppShell><PageHeader eyebrow={`${account.marketplace} consignment`} title="Upload and preview" description="No work tasks are created until an authorized owner or manager activates the reviewed batch." />
    ...
    <form action={uploadConsignmentAction} className="mx-auto max-w-2xl space-y-4 rounded-md border bg-white p-5 shadow-sm">
      ...
      <label className="block text-sm font-bold">{amazon?"Amazon shipment, listings, catalog files, or ZIP":"Consignment CSV or ZIP"}<input name="file" type="file" multiple={amazon} accept={amazon?".csv,.tsv,.txt,.xlsx,.xlsm,.zip":".csv,.zip,.txt"} required className="mt-1 block min-h-11 w-full rounded-md border p-2" /></label>
      <p className="text-xs text-slate-500">{amazon?"Classification uses header signatures, not filenames. XLSM cell data is read without executing macros or formulas.":"ZIP may include Labels.csv, Quality_Check_*.csv, and README.txt. These are private references only and create no workflow actions."}</p>
