...
export default async function ProductDetailPage({ params }: { params: Promise<{ listingId: string }> }) {
  ...
  const locks=(()=>{try{const parsed=JSON.parse(listing.manualLocksJson??"{}");return new Set<string>(Array.isArray(parsed)?parsed.filter((field):field is string=>typeof field==="string"):Object.entries(parsed&&typeof parsed==="object"?parsed:{}).flatMap(([field,locked])=>locked===true?[field]:[]));}catch{return new Set<string>();}})();
  return <AppShell><PageHeader eyebrow={`${listing.marketplace} / ${account.accountDisplayName??account.name}`} title={listing.productTitle??listing.sellerSkuId} description="Account-scoped product identity and optional processing configuration." action={{href:"/owner/product-inventory",label:"Back to inventory"}}/><div className="mb-4"><Link href={`/owner/product-inventory/${listing.id}/edit`} className="inline-flex min-h-11 items-center rounded-md bg-slate-950 px-4 font-bold text-white">Edit Listing</Link></div>
    <div className="grid gap-4 lg:grid-cols-[20rem_1fr]"><section className="rounded-md border bg-white p-4"><ProductImage src={listing.mainImageUrl} alt={listing.productTitle??listing.sellerSkuId} size="lg" showBadge/><h2 className="mt-4 font-black">Listing identity</h2><dl className="mt-2 space-y-2 text-sm"><Row label="Seller SKU" value={listing.sellerSkuId}/><Row label="Internal SKU" value={listing.sku}/><Row label="FSN" value={listing.fsn}/><Row label="Listing ID" value={listing.listingId}/><Row label="Status" value={listing.listingStatus}/><Row label="Category" value={listing.liveCategory??listing.subCategory}/><Row label="Brand" value={listing.liveBrand}/></dl></section>
      ...
      <section className="rounded-md border bg-white p-4"><h2 className="font-black">Manual catalog locks</h2><p className="mt-1 text-sm text-slate-600">Locked fields keep the current owner-reviewed value during future automated refreshes.</p><form action={saveCatalogFieldLocksAction} className="mt-3"><input type="hidden" name="marketplaceListingId" value={listing.id}/><input type="hidden" name="expectedUpdatedAt" value={listing.updatedAt.toISOString()}/><input type="hidden" name="clientRequestId" value={randomUUID()}/><div className="grid gap-2 sm:grid-cols-2">{LOCKABLE_CATALOG_FIELDS.map(field=><label key={field} className="flex min-h-11 items-center gap-2 rounded border p-2 text-sm"><input type="checkbox" name="lockedField" value={field} defaultChecked={locks.has(field)}/>{field}</label>)}</div><button className="mt-3 min-h-11 rounded-md bg-slate-900 px-4 py-2 font-bold text-white">Save field locks</button></form></section>
