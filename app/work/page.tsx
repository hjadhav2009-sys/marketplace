...
export default async function WorkHubPage(){const user=await requireUser();const account=await requireAccount(user);const counts=await getWorkHubCounts(user,account.id);const cards=[
 {show:hasWorkPermission(user,"canPick")||hasWorkPermission(user,"canMark")||hasWorkPermission(user,"canPack"),href:"/work/scan",title:"Universal Scan / Pack",stage:null},
