...
export async function GET(request:Request,{params}:{params:Promise<{taskId:string}>}){
 ...
 const taskAccount=await prisma.workTask.findFirst({where:{id:taskId,sourceType:"CONSIGNMENT",stage:"MARK"},select:{accountId:true}});if(!taskAccount)return Response.json({error:"Marking asset unavailable."},{status:404});await assertWorkerAccountAccess(user.id,taskAccount.accountId);
 const task=await prisma.workTask.findFirst({where:{id:taskId,accountId:taskAccount.accountId,sourceType:"CONSIGNMENT",stage:"MARK",status:{in:["READY","IN_PROGRESS"]}},include:{consignmentLine:{include:{markingAsset:{include:{files:{where:{activeVersion:true,attachmentType},take:1}}}}}}});
