...
export function BoundedMarketplaceListingForm({ action, issueId, issueVersion, clientRequestId, marketplace, sellerSku, knownIdentifiers, profiles, contextFields = {}, showMinimalAction = true }: Props) {
  ...
  return <form action={action} className="space-y-4 overflow-hidden">
    ...
    {Object.entries(attributeValues).filter(([key, value]) => value && !visibleKeys.has(key)).map(([key, value]) => <input key={key} type="hidden" name={`attribute:${key}`} value={value}/>)}
    ...
    {profiles.length ? <Surface padding="compact"><Field id="catalog-profile" label="Marketplace template" help="Only this account/global profile is used for advanced field validation.">{(attributes) => <select {...attributes} name="profileId" value={profileId} onChange={(event) => { setProfileId(event.target.value); setAttributeValues({}); }} className={fieldControlStyles()}><option value="">Common fields only</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select>}</Field></Surface> : null}
    ...
    <div className="sticky bottom-3 z-10 flex flex-wrap gap-2 rounded-md border border-slate-200 bg-white p-3 shadow-lg"><SubmitButton pendingText="Saving and releasing work...">Create full listing</SubmitButton>{showMinimalAction ? <button name="resolutionAction" value="CREATE_MINIMAL" className={buttonStyles({ variant: "secondary" })}>Create minimal listing</button> : null}</div>
  </form>;
}
