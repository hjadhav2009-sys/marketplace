...
export async function MissingListingResolutionWorkspace({ params, searchParams }: {
  ...
  return <AppShell><div className="mx-auto max-w-5xl">
    ...
    <nav aria-label="Resolution method" className="mb-4 grid gap-2 sm:grid-cols-3">{([['link','Link existing','Use an exact account listing'],['minimal','Create minimal','Identity now; enrich later'],['full','Create full','Add complete catalog detail']] as const).map(([value,label,description]) => <Link key={value} href={modeHref(issue.id, value)} className={`min-h-16 rounded-md border p-3 ${mode === value ? "border-berry bg-pink-50" : "border-slate-200 bg-white hover:bg-stone-50"}`}><span className="block font-semibold text-slate-950">{label}{mode === value ? <span className="sr-only"> (selected)</span> : null}</span><span className="mt-1 block text-xs text-slate-600">{description}</span></Link>)}</nav>
    ...
    {mode === "minimal" ? <Surface><h2 className="text-lg font-semibold">Create minimal account listing</h2><p className="mt-1 text-sm leading-6 text-slate-600">Creates only the protected seller identity and optional title, then releases this retained row through the normal service.</p><form action={resolveMissingListingAction} className="mt-4 grid gap-4 sm:grid-cols-2"><input type="hidden" name="issueId" value={issue.id}/><input type="hidden" name="expectedIssueVersion" value={issue.version}/><input type="hidden" name="clientRequestId" value={stableActionRequestId("missing-minimal", issue.id, issue.version)}/><input type="hidden" name="resolutionAction" value="CREATE_MINIMAL"/><Fact label="Protected Seller SKU" value={sellerSku}/><label className="font-semibold">Title (optional)<input className="ui-field-control mt-1 w-full" name="productTitle" maxLength={500}/></label><label className="flex min-h-11 items-center gap-2 sm:col-span-2"><input type="checkbox" name="manualLocked" defaultChecked/> Protect entered values from automated refresh</label><div className="sm:col-span-2"><SubmitButton pendingText="Creating and releasing...">Create minimal listing</SubmitButton></div></form></Surface> : null}
    {mode === "full" ? <BoundedMarketplaceListingForm action={resolveMissingListingAction} issueId={issue.id} issueVersion={issue.version} clientRequestId={stableActionRequestId("missing-full", issue.id, issue.version)} marketplace={account.marketplace} sellerSku={sellerSku} knownIdentifiers={knownIdentifiers} profiles={parsedProfiles} showMinimalAction={false}/> : null}
  </div></AppShell>;
}
