...
export function WorkProcessFlow({ currentStage, route, onActivate, fallback = false }: { currentStage: WorkStage; route: string | null | undefined; onActivate?: () => void; fallback?: boolean }) {
  ...
  const degraded = currentIndex === -1;
  const content = <>
    <span className="block text-[0.68rem] font-semibold uppercase tracking-wide text-slate-500">Process flow</span>
    <span className="mt-1 flex min-w-0 flex-wrap items-center gap-x-1 text-sm" aria-label={degraded ? `${humanProcessRoute(route)}. Current stage ${LABELS[currentStage]} is not represented in this flow.` : `${humanProcessRoute(route)}. Current stage ${LABELS[currentStage]}.`}>
      {stages.map((stage, index) => <span key={stage} className="contents">
        {index ? <span aria-hidden="true" className="text-slate-400">→</span> : null}
        <span className={!degraded && index < currentIndex ? "font-medium text-teal-700" : !degraded && index === currentIndex ? "font-semibold text-slate-950" : "font-medium text-slate-500"}>
          {LABELS[stage]}{!degraded && index < currentIndex ? <span className="sr-only"> completed</span> : null}
          {!degraded && index < currentIndex ? <svg aria-hidden="true" viewBox="0 0 16 16" className="ml-1 inline h-3.5 w-3.5 fill-none stroke-current" strokeWidth="2"><path d="m3 8 3 3 7-7" /></svg> : null}
        </span>
      </span>)}
    ...
    {degraded ? <span className="mt-1 block text-xs font-medium text-amber-800">Current stage: {LABELS[currentStage]} · not represented in this flow</span> : null}
    {fallback ? <span className="mt-1 block text-xs text-slate-500">System fallback</span> : null}
  </>;

  return onActivate
    ? <button type="button" onClick={onActivate} className="min-h-11 w-full rounded-md px-2 py-1.5 text-left hover:bg-slate-50" data-route-degraded={degraded || undefined}>{content}</button>
    : <div className="min-h-11 px-2 py-1.5" data-route-degraded={degraded || undefined}>{content}</div>;
}
