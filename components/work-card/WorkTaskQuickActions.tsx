...
export function WorkTaskQuickActions({ model, canProgress, canReportProblem }: { model: WorkTaskQuickModel; canProgress: boolean; canReportProblem: boolean }) {
  ...
  return <>
    {canProgress && model.stage !== "PACK" && model.completed < model.required ? <button type="button" onClick={partial} className={buttonStyles({ variant: "secondary", className: "w-full" })}>Partial Quantity</button> : null}
...
function TaskPartialQuantity({ model }: { model: WorkTaskQuickModel }) {
  ...
  return <form action={setTaskProgressAction} className="grid gap-4">
    ...
    <input type="hidden" name="taskId" value={model.taskId}/><input type="hidden" name="expectedQuantity" value={model.completed}/><input type="hidden" name="clientRequestId" value={requestId}/><input type="hidden" name="returnPath" value={model.returnPath}/>
    <Field id={`task-target-${model.taskId}`} label="Exact completed quantity" required>{(attributes) => <input {...attributes} name="targetQuantity" type="number" min={model.completed} max={model.required} step="1" defaultValue={model.completed} className={fieldControlStyles()} />}</Field>
