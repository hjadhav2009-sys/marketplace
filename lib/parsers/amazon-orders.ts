...
export function parseAmazonOrderReport(content:string,fileName="amazon-orders.csv"):ParsedOrderImportRow[]{
 ...
 for(const required of ["orderNo","trackingId","sku","qty"] as const)if(index(headers,aliases[required])<0)throw new Error(`Amazon order report is missing ${required}.`);
 return records.slice(1).flatMap((row,offset)=>{if(row.every(cell=>!cell.trim()))return[];const quantity=value(row,headers,"qty");if(!/^\d+$/.test(quantity)||Number(quantity)<=0)throw new Error(`Amazon order row ${offset+2} has an invalid quantity.`);const trackingId=value(row,headers,"trackingId");if(!trackingId)throw new Error(`Amazon order row ${offset+2} is missing Tracking ID.`);return[{rowNumber:offset+2,awb:trackingId,trackingId,orderNo:value(row,headers,"orderNo"),orderItemId:value(row,headers,"orderItemId")||null,shipmentId:value(row,headers,"shipmentId")||null,sku:value(row,headers,"sku"),qty:Number(quantity),productDescription:value(row,headers,"title")||null,courier:value(row,headers,"courier")||null,city:value(row,headers,"city")||null,state:value(row,headers,"state")||null}];});
}
