...
async function seed() {
  ...
  const c3aCases = [
    ...
    { id: "stage4-c3a-single-destination", row: 2, sku: "STAGE-C3A-SINGLE-DESTINATION", title: "Synthetic C3A Mark with one compatible completion path", actualStages: ["PICK", "MARK", "ASSEMBLE"] as WorkStage[] },
  ];
  for (const item of c3aCases) {
    ...
    const c3aRouteSnapshot = JSON.stringify({ version: 3, ...provenance, routeRecommendation: "PICK_MARK_PACK", routeRecommendationSource: "EXPLICIT_PRODUCT_RULE", hasExplicitSavedRoute: true, actualProcessRoute: "PICK_MARK_PACK", actualStages: item.actualStages, currentStage: "MARK", completedStages: ["PICK"], decision: "SYNTHETIC_C3A_MARK_SAFETY" });
    ...
    await createTask({ id: `${item.id}-pack`, consignmentLineId: item.id, sourceType: "CONSIGNMENT", stage: "PACK", sequence: 3, status: "LOCKED", quantity: 6, assigned: users[6].id, sku: item.sku, title: item.title, route: "PICK_MARK_PACK", workCardSnapshotJson: cardSnapshot, routeSnapshotJson: c3aRouteSnapshot });
  }
