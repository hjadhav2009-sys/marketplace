...
async function seed() {
  ...
  for (const item of c3aCases) {
    ...
    const preselectedAssembly = item.id === "stage4-c3a-single-destination";
    const c3aRouteSnapshot = JSON.stringify({ ...provenance, version: 2, routeVersion: 2, recommendedStages: ["PICK", "MARK", "PACK"], actualProcessRoute: preselectedAssembly ? "PICK_MARK_ASSEMBLE_PACK" : null, actualStages: item.actualStages, currentStage: "MARK", completedStages: ["PICK"], decisions: [], decision: "SYNTHETIC_C3A_MARK_SAFETY" });
