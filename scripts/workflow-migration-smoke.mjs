...
const root=process.cwd(),tmp=resolve(root,".codex-tmp"),migrations=resolve(root,"prisma","migrations"),latest="20260711000300_consignment_worker_workflow";mkdirSync(tmp,{recursive:true});const entries=readdirSync(migrations,{withFileTypes:true}).filter((e)=>e.isDirectory()).map((e)=>e.name).sort();
...
`);apply(old.db,latest);
assert.equal(old.db.prepare("SELECT count(*) count FROM ConsignmentImportFile WHERE consignmentBatchId='batch' AND fileType='SOURCE_UPLOAD' AND isCurrentSource=true").get().count,1);
...
old.db.exec("INSERT INTO WorkActionLog (id,accountId,taskId,actorUserId,action,clientRequestId,createdAt) VALUES ('log1','acct','pick','owner','TASK_CLAIMED','request-1',CURRENT_TIMESTAMP)");
assert.throws(()=>old.db.exec("INSERT INTO WorkActionLog (id,accountId,taskId,actorUserId,action,clientRequestId,createdAt) VALUES ('log2','acct','pick','owner','TASK_CLAIMED','request-1',CURRENT_TIMESTAMP)"),/UNIQUE/i);
