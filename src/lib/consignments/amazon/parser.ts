...
const DEPRIORITIZED_SHEETS=/^(instructions?|data definitions?|valid values?|images?|examples?|dropdowns?)$/i;
const PREFERRED_SHEETS=/template|upload|product|inventory/i;
...
function tableFromRows(sheet:string,records:string[][]):AmazonParsedTable{
  const layout=resolveAmazonHeaderLayout(records);const headers=layout.headers;const classification=layout.classification;const rows:AmazonConsignmentSourceRow[]=[];const issues:AmazonParserIssue[]=[];
  ...
  return{sheet,headers,profile:classification.profile,fileType:classification.fileType,confidence:classification.confidence,headerRow:layout.headerRow,labelRow:layout.labelRow,dataRow:layout.dataRow,rows,issues};
}

async function workbookTables(buffer:Buffer){if(buffer.length>AMAZON_WORKBOOK_MAX_BYTES)throw new Error("Amazon workbook exceeds the configured size limit.");const workbook=new ExcelJS.Workbook();try{await workbook.xlsx.load(buffer as never);}catch{throw new Error("Amazon workbook is corrupt, encrypted, or unsupported.");}if(workbook.worksheets.length>AMAZON_WORKBOOK_MAX_SHEETS)throw new Error("Amazon workbook has too many sheets.");const tables:AmazonParsedTable[]=[];let totalCells=0;for(const sheet of workbook.worksheets){if(sheet.rowCount>AMAZON_WORKBOOK_MAX_ROWS_PER_SHEET||sheet.columnCount>AMAZON_WORKBOOK_MAX_COLUMNS)throw new Error("Amazon workbook exceeds row or column limits.");totalCells+=sheet.rowCount*sheet.columnCount;if(totalCells>AMAZON_WORKBOOK_MAX_TOTAL_CELLS)throw new Error("Amazon workbook contains too many cells.");const records=Array.from({length:sheet.rowCount},(_,rowIndex)=>Array.from({length:Math.min(sheet.columnCount,AMAZON_WORKBOOK_MAX_COLUMNS)},(_,columnIndex)=>cellText(sheet.getRow(rowIndex+1).getCell(columnIndex+1).value)));if(records.some((record)=>record.some(Boolean))){const table=tableFromRows(clean(sheet.name,100)??"Sheet",records);const sheetPenalty=DEPRIORITIZED_SHEETS.test(sheet.name)?-40:PREFERRED_SHEETS.test(sheet.name)?5:0;tables.push({...table,confidence:Math.max(0,table.confidence+sheetPenalty)});}}return tables;}
