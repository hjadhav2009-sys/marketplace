...
export type AmazonParsedTable = { sheet: string; sheetUsage: AmazonSheetUsage; sheetPriority: number; headers: string[]; technicalHeaders?: string[]; humanHeaders?: string[]; profile: AmazonSourceProfile; fileType: ConsignmentImportFileType; confidence: number; headerRow: number; technicalHeaderRow?: number; labelRow?: number; humanHeaderRow?: number; dataRow: number; rows: AmazonConsignmentSourceRow[]; issues: AmazonParserIssue[]; sourceRowCount?:number; skippedZeroRows?:number };
