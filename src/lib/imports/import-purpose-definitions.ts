...
import { PRODUCT_CATALOG_REGISTRIES } from "./canonical-field-registry";
...
const productCatalogDefinitions:ImportPurposeDefinition[]=PRODUCT_CATALOG_REGISTRIES.map(registry=>({marketplace:registry.marketplace,purpose:"PRODUCT_CATALOG",label:registry.label,fields:registry.fields.map(item=>field(item.mappingKey,item.label,item.targetHeader,item.required))}));
