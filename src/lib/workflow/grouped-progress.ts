...
import { assertValidStageTransition, createWorkRouteSnapshot, parseWorkRouteSnapshot, recommendedNextStage, routeFingerprint } from "./dynamic-route";
...
import { advanceGroupedRouteSnapshot, assertPreparedPreselectedDownstreamTask, resolveGroupedCompletionDestination } from "./grouped-completion-destination";
...
function isTransient(error: unknown) {
  ...
  return error instanceof Prisma.PrismaClientKnownRequestError && ["P1008", "P2002", "P2028", "P2034"].includes(error.code) || /database is locked|write conflict|transaction.*(?:closed|conflict|timeout)/i.test(message);
}
...
async function routeFinishedMember(tx:Transaction,input:{task:MemberTask;actorUserId:string;accountId:string;stage:WorkStage;clientRequestId:string;requestFingerprint:string}&RouteInput){
  ...
  const destination=resolveGroupedCompletionDestination({snapshot,currentStage:input.stage,requestedNextStage:input.nextStage,useRecommendedNextStage:input.useRecommendedNextStage!==false});
  const next=destination.nextStage;
  if(!destination.preselected)assertValidStageTransition(snapshot,input.stage,next);
  const routedSnapshot=advanceGroupedRouteSnapshot({snapshot,currentStage:input.stage,nextStage:next,actorUserId:input.actorUserId,useRecommendedNextStage:input.useRecommendedNextStage!==false,preselected:destination.preselected});
  ...
  const sourceContext=await resolveRouteSourceContext(tx,{accountId:input.accountId,sourceType:task.sourceType,sourceId,workCardSnapshotJson:task.workCardSnapshotJson,routeSnapshotJson:task.routeSnapshotJson});
  ...
  if(next==="MARK"||next==="ASSEMBLE"){
    ...
    const resolution=await resolveRouteStageMetadata(tx,{accountId:input.accountId,actorUserId:input.actorUserId,sourceType:task.sourceType,sourceId,route:choice,requestFingerprint:input.requestFingerprint,workerNote,workCardSnapshotJson:task.workCardSnapshotJson,routeSnapshotJson:task.routeSnapshotJson});
  ...
  return{next,routedSnapshot,decisionType:policy.decisionType,reason,workerNote,missingInstructionStage,preselected:false};
}
...
export async function setGroupedProgress(input:{actorUserId:string;selectedAccountId:string;sourceType:GroupedWorkSource;stage:WorkStage;groupKey:string;expectedGroupVersion:string;targetCompletedQuantity:number;clientRequestId:string}&RouteInput,client:Client=prisma){
  ...
  const workerNote=sanitizeRouteNote(input.workerNote),requestFingerprint=routeFingerprint({groupKey:input.groupKey,expectedGroupVersion:input.expectedGroupVersion,targetCompletedQuantity:input.targetCompletedQuantity,stage:input.stage,sourceType:input.sourceType,nextStage:input.nextStage??null,useRecommended:input.useRecommendedNextStage!==false,routeReason:input.routeReason??null,routeOtherReason:input.routeOtherReason??null,workerNote,confirmMissingInstructions:Boolean(input.confirmMissingInstructions)});
  return retry(client,async tx=>{
    const {user}=await assertWorkerAccountAccess(input.actorUserId,input.selectedAccountId,tx);if(!userCanMutateStage(user,input.stage))throw new Error(`${input.stage} permission is required.`);
    const receipt=await beginWorkflowActionReceipt<{groupKey:string;targetCompletedQuantity:number;completedMembers?:number;partialMembers?:number;idempotent:boolean}>(tx,{accountId:input.selectedAccountId,actorUserId:user.id,requestKind:"GROUP_PROGRESS",clientRequestId:input.clientRequestId,requestFingerprint,sourceType:input.sourceType,stage:input.stage,originalGroupKey:input.groupKey});if(receipt.replay)return{...receipt.replay,idempotent:true};
    ...
    if(replay.length){const metadata=replay.map(log=>{try{return JSON.parse(log.metadataJson??"")as{requestFingerprint?:string;groupTarget?:number};}catch{return{};}});if(metadata.some(item=>item.requestFingerprint!==requestFingerprint))throw new Error("Request ID was already used for different progress.");return completeWorkflowActionReceipt(tx,receipt.receiptId,{groupKey:input.groupKey,targetCompletedQuantity:metadata[0].groupTarget??input.targetCompletedQuantity,idempotent:true});}
    ...
    await refreshAffectedWorkGroups({accountId:input.selectedAccountId,sourceType:input.sourceType,stages:[input.stage,...routedStages],taskIds:tasks.map(task=>task.id),orderIds:tasks.flatMap(task=>task.orderId?[task.orderId]:[]),consignmentLineIds:tasks.flatMap(task=>task.consignmentLineId?[task.consignmentLineId]:[])},tx);
    return completeWorkflowActionReceipt(tx,receipt.receiptId,{groupKey:card.groupKey,targetCompletedQuantity:input.targetCompletedQuantity,completedMembers,partialMembers,idempotent:false});
  });
}
...
export async function completeSelectedGroupMembers(input:{actorUserId:string;selectedAccountId:string;sourceType:GroupedWorkSource;stage:WorkStage;groupKey:string;expectedGroupVersion:string;selectedTaskIds:string[];clientRequestId:string}&RouteInput,client:Client=prisma){
  ...
  const selectedIds=[...new Set(input.selectedTaskIds.filter(Boolean))].sort();if(!selectedIds.length)throw new Error("Select at least one eligible member.");if(selectedIds.length>50)throw new Error("Select no more than 50 visible members at once.");
  const workerNote=sanitizeRouteNote(input.workerNote),requestFingerprint=routeFingerprint({groupKey:input.groupKey,expectedGroupVersion:input.expectedGroupVersion,selectedIds,stage:input.stage,sourceType:input.sourceType,nextStage:input.nextStage??null,useRecommended:input.useRecommendedNextStage!==false,routeReason:input.routeReason??null,routeOtherReason:input.routeOtherReason??null,workerNote,confirmMissingInstructions:Boolean(input.confirmMissingInstructions)});
  return retry(client,async tx=>{
    const {user}=await assertWorkerAccountAccess(input.actorUserId,input.selectedAccountId,tx);if(!userCanMutateStage(user,input.stage))throw new Error(`${input.stage} permission is required.`);
    const receipt=await beginWorkflowActionReceipt<{groupKey:string;completedMemberCount:number;completedQuantity:number;idempotent:boolean}>(tx,{accountId:input.selectedAccountId,actorUserId:user.id,requestKind:"GROUP_EXACT",clientRequestId:input.clientRequestId,requestFingerprint,sourceType:input.sourceType,stage:input.stage,originalGroupKey:input.groupKey});if(receipt.replay)return{...receipt.replay,idempotent:true};
    const resolved=await getGroupedWork({actorUserId:input.actorUserId,accountId:input.selectedAccountId,stage:input.stage,sourceType:input.sourceType,pageSize:1,includeMemberIds:false,targetGroupKey:input.groupKey},tx);
    ...
    const tasks=await tx.workTask.findMany({where:{id:{in:selectedIds},accountId:input.selectedAccountId,sourceType:input.sourceType,stage:input.stage,status:{in:["READY","IN_PROGRESS"]},workGroupMembership:{is:{groupKey:input.groupKey}}},include:{consignmentLine:{select:{processRoute:true}}},orderBy:[{createdAt:"asc"},{id:"asc"}]});
    if(tasks.length!==selectedIds.length||tasks.some(task=>task.assignedUserId&&task.assignedUserId!==input.actorUserId&&resolved.user.role!=="OWNER"))throw new Error("One or more selected members are not eligible in this work group.");
    const now=new Date(),routedStages=new Set<WorkStage>();for(const task of tasks){const routeResult=await routeFinishedMember(tx,{...input,task,accountId:input.selectedAccountId,requestFingerprint});if(routeResult.next)routedStages.add(routeResult.next);const changed=await tx.workTask.updateMany({where:{id:task.id,version:task.version,status:task.status,completedQuantity:task.completedQuantity,assignedUserId:task.assignedUserId},data:{completedQuantity:task.requiredQuantity,status:"COMPLETED",assignedUserId:task.assignedUserId??input.actorUserId,startedAt:task.startedAt??now,startedByUserId:task.startedByUserId??input.actorUserId,completedAt:now,completedByUserId:input.actorUserId,routeSnapshotJson:JSON.stringify(routeResult.routedSnapshot),version:{increment:1}}});if(changed.count!==1)throw new Error("Selected work changed. No members were completed.");await tx.workActionLog.create({data:{accountId:input.selectedAccountId,taskId:task.id,actorUserId:input.actorUserId,action:"TASK_COMPLETED",requestKind:"SET_PROGRESS",clientRequestId:input.clientRequestId,quantityBefore:task.completedQuantity,quantityAfter:task.requiredQuantity,metadataJson:JSON.stringify({requestFingerprint,groupKey:card.groupKey,exactSelection:true,nextStage:routeResult.next??null,preselected:routeResult.preselected})}});}
    await tx.auditLog.create({data:{userId:input.actorUserId,accountId:input.selectedAccountId,action:"WORK_GROUP_MEMBERS_COMPLETED",entityType:"WorkGroup",entityId:card.groupKey,metadata:JSON.stringify({sourceType:input.sourceType,stage:input.stage,memberCount:tasks.length,quantity:tasks.reduce((sum,task)=>sum+task.requiredQuantity-task.completedQuantity,0)})}});await tx.workChangeEvent.create({data:{accountId:input.selectedAccountId,eventType:"GROUP_PROGRESS_SET",sourceType:input.sourceType,stage:input.stage,groupKey:card.groupKey}});await refreshAffectedWorkGroups({accountId:input.selectedAccountId,sourceType:input.sourceType,stages:[input.stage,...routedStages],taskIds:tasks.map(task=>task.id),orderIds:tasks.flatMap(task=>task.orderId?[task.orderId]:[]),consignmentLineIds:tasks.flatMap(task=>task.consignmentLineId?[task.consignmentLineId]:[])},tx);return completeWorkflowActionReceipt(tx,receipt.receiptId,{groupKey:card.groupKey,completedMemberCount:tasks.length,completedQuantity:tasks.reduce((sum,task)=>sum+task.requiredQuantity-task.completedQuantity,0),idempotent:false});
  });
}
