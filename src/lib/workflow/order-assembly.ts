...
async function createTaskIfMissing(tx: Transaction, input: { order: AssemblyOrder; metadata: ReturnType<typeof buildOrderAssemblyMetadata> }) {
  const existing = await tx.workTask.findUnique({ where: { orderId_stage: { orderId: input.order.id, stage: "ASSEMBLE" } } });
...
export async function createAutomaticAssemblyTasksAfterPick(tx: Transaction, input: { actorUserId: string; accountId: string; orders: AssemblyOrder[] }) {
  ...
  for (const order of input.orders) {
    ...
    if (policy?.state === "ASSEMBLY_REQUIRED") {
      const result = await createTaskIfMissing(tx, { order, metadata: taskMetadata(order, policy, input.actorUserId) });
      if (result.created) {
        createdCount += 1;
...
export async function sendOrderToAssembly(input: { actorUserId: string; accountId: string; orderId: string; manualTitle?: string; manualInstructions?: string; manualImageUrl?: string; clientRequestId?: string }, client: Client = prisma) {
  ...
  try {
    return await client.$transaction(async (tx) => {
      ...
      const existing = await tx.workTask.findUnique({ where: { orderId_stage: { orderId: order.id, stage: "ASSEMBLE" } } });
      if (existing) return { task: existing, created: false, idempotent: true };
      ...
      const result = await createTaskIfMissing(tx, { order, metadata });
      ...
      return { task: result.task, created: result.created, idempotent: !result.created };
    });
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      const task = await client.workTask.findUnique({ where: { orderId_stage: { orderId: input.orderId, stage: "ASSEMBLE" } } });
...
export async function reportOrderAssemblyProblem(input: { taskId: string; accountId: string; actorUserId: string; reason: string; note?: string; expectedStatus: string; clientRequestId?: string }, client: Client = prisma) {
  ...
  return client.$transaction(async (tx) => {
    ...
    if (task.assignedUserId && task.assignedUserId !== user.id && user.role !== "OWNER") throw new Error("This assembly task was taken by another worker.");
    if (!(["READY", "IN_PROGRESS"] as string[]).includes(task.status) || task.status !== input.expectedStatus) throw new Error("Assembly changed; refresh before reporting a problem.");
    const changed = await tx.workTask.updateMany({ where: { id: task.id, status: task.status, assignedUserId: task.assignedUserId }, data: { statusBeforeProblem: task.status, status: "PROBLEM", assignedUserId: task.assignedUserId ?? user.id, problemReason: input.reason, problemReportedAt: new Date(), problemReportedByUserId: user.id } });
...
export async function resolveOrderAssemblyProblem(input: { taskId: string; accountId: string; actorUserId: string; resolutionNote: string; clientRequestId?: string }, client: Client = prisma) {
  ...
  return client.$transaction(async (tx) => {
    const { user, task } = await orderAssemblyTaskForMutation(tx, input);
    if (user.role !== "OWNER") throw new Error("Only an owner can resolve assembly problems.");
    if (task.status !== "PROBLEM") throw new Error("Assembly problem is unavailable.");
    const restored = task.completedQuantity > 0 ? "IN_PROGRESS" as const : "READY" as const;
    await tx.workTask.update({ where: { id: task.id }, data: { status: restored, problemResolutionNote: input.resolutionNote.trim().slice(0, 1_000), problemResolvedAt: new Date(), problemResolvedByUserId: user.id } });
    await actionLog(tx, { ...input, action: "TASK_PROBLEM_RESOLVED", requestKind: "RESOLVE_PROBLEM", note: input.resolutionNote });
    await tx.auditLog.create({ data: { userId: user.id, accountId: input.accountId, action: "ORDER_ASSEMBLY_PROBLEM_RESOLVED", entityType: "WorkTask", entityId: task.id, metadata: JSON.stringify({ restoredStatus: restored }) } });
    return { taskId: task.id, status: restored, idempotent: false };
  });
}
...
export async function skipOrderAssemblyTask(input: { taskId: string; accountId: string; actorUserId: string; reason: string; clientRequestId?: string }, client: Client = prisma) {
  ...
  return client.$transaction(async (tx) => {
    ...
    if (task.status === "SKIPPED") return { taskId: task.id, status: task.status, idempotent: true };
    ...
    return { taskId: task.id, status: "SKIPPED" as const, idempotent: false };
  });
}
...
export async function reassignOrderAssemblyTask(input: { taskId: string; accountId: string; actorUserId: string; assignedUserId: string | null; clientRequestId?: string }, client: Client = prisma) {
  return client.$transaction(async (tx) => {
    ...
    return { taskId: task.id, assignedUserId: input.assignedUserId };
  });
}
