...
export async function reportOrderMarkingProblem(input:{taskId:string;accountId:string;actorUserId:string;expectedStatus:string;reason:string;note?:string;clientRequestId?:string},client:Client=prisma){
  const allowed=new Set(["WRONG_PRODUCT","MARKING_FILE_MISSING","MARKING_FILE_WRONG","MARKING_FAILED","DAMAGED_PRODUCT","QUANTITY_SHORT","OTHER"]);if(!allowed.has(input.reason))throw new Error("Select a valid marking problem reason.");const requestFingerprint=fingerprint({expectedStatus:input.expectedStatus,reason:input.reason,note:input.note?.trim()||null});
  return recover({clientRequestId:input.clientRequestId,mutate:()=>client.$transaction(async tx=>{const{user,task}=await taskForMutation(tx,input);if(!hasWorkPermission(user,"canMark")||(user.role!=="OWNER"&&!user.canReportProblem))throw new Error("Marking problem reporting permission is required.");const prior=await replay(tx,{...input,requestKind:"REPORT_PROBLEM",requestFingerprint});if(prior)return prior;if(task.assignedUserId&&task.assignedUserId!==user.id&&user.role!=="OWNER")throw new Error("This marking task was taken by another worker.");if(task.status!==input.expectedStatus||!["READY","IN_PROGRESS"].includes(task.status))throw new Error("Marking changed; refresh before reporting a problem.");
    const changed=await tx.workTask.updateMany({where:{id:task.id,status:task.status,assignedUserId:task.assignedUserId},data:{statusBeforeProblem:task.status,status:"PROBLEM",assignedUserId:task.assignedUserId??user.id,problemReason:input.reason,problemReportedAt:new Date(),problemReportedByUserId:user.id}});if(changed.count!==1)throw new Error("Marking changed; refresh before reporting a problem.");await tx.workActionLog.create({data:{accountId:input.accountId,taskId:task.id,actorUserId:user.id,action:"TASK_PROBLEM_REPORTED",requestKind:input.clientRequestId?"REPORT_PROBLEM":null,clientRequestId:input.clientRequestId||null,note:input.note?.trim().slice(0,1000)||null,metadataJson:JSON.stringify({requestFingerprint,reason:input.reason})}});return{taskId:task.id,status:"PROBLEM" as const,idempotent:false};}),replay:()=>client.$transaction(async tx=>{const{user,task}=await taskForMutation(tx,input);if(!hasWorkPermission(user,"canMark")||(user.role!=="OWNER"&&!user.canReportProblem))throw new Error("Marking problem reporting permission is required.");return replay(tx,{...input,taskId:task.id,requestKind:"REPORT_PROBLEM",requestFingerprint});})});
}
