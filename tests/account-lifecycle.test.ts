...
import { assertAccountMarketplaceChangeAllowed, setAccountActiveSafely, updateAccountDetailsSafely } from "../lib/account-lifecycle";
...
try{
 await db.account.create({data:{id:"empty",name:"Empty",code:"EMPTY",marketplace:"FLIPKART"}});await assertAccountMarketplaceChangeAllowed("empty","AMAZON",db);await updateAccountDetailsSafely({accountId:"empty",companyName:"Synthetic",marketplace:"AMAZON",name:"Empty Amazon",code:"EMPTY-AMZ"},db);assert.equal((await db.account.findUniqueOrThrow({where:{id:"empty"}})).marketplace,"AMAZON");
