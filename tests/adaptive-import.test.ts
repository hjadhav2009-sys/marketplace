...
try{
 ...
 const runner=readFileSync(resolve("src/lib/import-jobs/runner.ts"),"utf8"),mappingPage=readFileSync(resolve("app/owner/imports/[jobId]/mapping/page.tsx"),"utf8"),mappingAction=readFileSync(resolve("app/owner/imports/[jobId]/mapping/actions.ts"),"utf8");assert.match(runner,/markImportJobNeedsMapping/);assert.match(runner,/findHeaderProfile/);assert.match(mappingPage,/Map File Headers/);assert.match(mappingPage,/no re-upload is required/i);assert.match(mappingAction,/requireUser\(\["OWNER"\]\)/);assert.match(mappingAction,/resumeMappedImportJob/);
