...
assert.match(activation,/ACTIVATABLE_CONSIGNMENT_STATUSES = new Set\(\["REVIEW_REQUIRED", "READY_TO_ACTIVATE"\]\)/,"Activation lifecycle policy is explicit and shared");assert.match(activation,/status: candidate\.status/,"Activation claims the exact validated lifecycle state");assert.match(activation,/status: "ACTIVATING"/);assert.match(activation,/workTask\.createMany/);assert.doesNotMatch(activation,/sourceType: "ORDER"/,"Consignment activation never creates Order tasks");
