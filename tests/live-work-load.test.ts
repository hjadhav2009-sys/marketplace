...
try{
 ...
 const route=readFileSync(resolve("app/api/work/live/route.ts"),"utf8"),accessService=readFileSync(resolve("src/lib/workflow/live-work.ts"),"utf8"),client=readFileSync(resolve("app/work/LiveWorkRefresh.tsx"),"utf8"),handler=client.slice(client.indexOf("events.onmessage"),client.indexOf("events.onopen"));assert.match(route,/resolveLiveWorkAccess/);assert.match(accessService,/stagePermissionField/);assert.match(accessService,/marketplaceCapabilityEnabled/);assert.match(accessService,/assertWorkerAccountAccess/);assert.match(accessService,/canViewAllWork/);assert.match(route,/20000/);assert.match(route,/5000/);assert.doesNotMatch(handler,/router\.refresh/);assert.match(handler,/work-change/);assert.match(handler,/updateSummary/);assert.match(client,/work-summary-change/);assert.match(client,/30000/);assert.match(client,/scrollY/);assert.match(client,/\.focus\(\)/);
