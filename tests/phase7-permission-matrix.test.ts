...
const nav=readFileSync("lib/app-navigation.ts","utf8");for(const permission of ["canPick","canMark","canAssemble","canPack","canViewConsignments","canManageMarkingLibrary","canManageProcessRules"])assert.match(nav,new RegExp(`hasWorkPermission\\(user, "${permission}"\\)`),`Navigation checks ${permission}`);
