...
const image=readFileSync("components/ProductImage.tsx","utf8"),legacyCard=readFileSync("app/work/WorkTaskCard.tsx","utf8"),routeChoice=readFileSync("components/WorkRouteDialog.tsx","utf8");assert.match(image,/IntersectionObserver/);assert.match(image,/if \(!eligibleToLoad/);assert.match(legacyCard,/hasExplicitSavedRoute:Boolean\(provenance\?\.hasExplicitSavedRoute\)/);assert.match(routeChoice,/const reasonRequired/);assert.match(routeChoice,/name="routeOtherReason"/);assert.doesNotMatch(routeChoice,/<select[^>]+name="routeReason"/);
