...
const image=readFileSync("components/ProductImage.tsx","utf8"),legacyCard=readFileSync("app/work/WorkTaskCardView.tsx","utf8"),routeChoice=readFileSync("components/WorkRouteDialog.tsx","utf8");assert.match(image,/IntersectionObserver/);assert.match(image,/if \(!eligibleToLoad/);assert.match(legacyCard,/hasExplicitSavedRoute:\s*Boolean\(provenance\?\.hasExplicitSavedRoute\)/);assert.match(routeChoice,/const reasonRequired/);assert.match(routeChoice,/name="routeOtherReason"/);assert.match(routeChoice,/<select[^>]+name="routeReason"/);
