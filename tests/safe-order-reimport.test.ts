...
try{
 ...
 const untouched=await prisma.order.create({data:{id:"rollback-order",accountId:"account",batchId:conflictBatch.id,marketplace:"FLIPKART",awb:"FLIPKART:ORDER_ITEM:ITEM-ROLLBACK",shipmentId:"SHIP-ROLLBACK",orderItemId:"ITEM-ROLLBACK",sku:"ROLLBACK-SKU",qty:1,orderNo:"ROLLBACK"}});
 await prisma.$executeRawUnsafe(`CREATE TRIGGER fail_safe_order_reimport BEFORE INSERT ON "WorkTask" WHEN NEW."orderId" = 'rollback-order' BEGIN SELECT RAISE(ABORT, 'synthetic reimport rolled back'); END`);
 try{await assert.rejects(()=>importFlipkartOrderRows({rows:[row({"Shipment ID":"SHIP-ROLLBACK","ORDER ITEM ID":"ITEM-ROLLBACK",SKU:"ROLLBACK-CHANGED",Quantity:"4"})],fileName:"rollback.csv",account,user}),"A task-plan write failure aborts the complete Order reimport transaction");}finally{await prisma.$executeRawUnsafe("DROP TRIGGER IF EXISTS fail_safe_order_reimport");}
 const rolledBack=await prisma.order.findUniqueOrThrow({where:{id:untouched.id}});assert.equal(rolledBack.sku,"ROLLBACK-SKU");assert.equal(rolledBack.qty,1);assert.equal(await prisma.workTask.count({where:{orderId:untouched.id}}),0,"No partial task survives the failed reimport transaction");
