...
const groupedQuick = await read("components/work-card/GroupedQuickActions.tsx");
const taskQuick = await read("components/work-card/WorkTaskQuickActions.tsx");
const card = `${grouped}\n${groupedQuick}\n${workCard}\n${workQuantity}`;
...
assert.doesNotMatch(legacyTask,/RouteChoiceWithInstructionConfirmation/);assert.match(taskQuick,/canProgress && model\.stage !== "PACK"/,"Generic exact progress is explicitly excluded for Pack.");
