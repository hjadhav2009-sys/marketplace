...
process.env.STAGE3_STAGING_PORT = String(32_000 + (process.pid % 1_000));
...
assert.match(source, /commandFingerprint/); assertions += 1;
assert.match(source, /STAGING_START_ALREADY_IN_PROGRESS/); assertions += 1;
assert.match(source, /START_TIMEOUT_MS = 120_000/); assertions += 1;
assert.match(source, /state: "STOPPED"/); assertions += 1;
assert.match(source, /"RUNNING" : "MISMATCHED"/); assertions += 1;
assert.match(source, /taskkill\.exe/); assertions += 1;
assert.match(source, /MAX_LOG_BYTES/); assertions += 1;
...
assert.match(identityRoute, /sourceSha/); assertions += 1;
assert.match(identityRoute, /buildId/); assertions += 1;
assert.match(identityRoute, /commandFingerprint/); assertions += 1;
const packageJson = readFileSync(path.join(root, "package.json"), "utf8");
assert.match(packageJson, /"staging:restart"/); assertions += 1;
assert.match(packageJson, /"staging:health"/); assertions += 1;
