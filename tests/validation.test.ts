...
const productInventoryCard = readFileSync(join(repoRoot, "app", "owner", "product-inventory", "InventoryCard.tsx"), "utf8");
const productDetailPage = readFileSync(join(repoRoot, "app", "owner", "product-inventory", "[listingId]", "page.tsx"), "utf8");
const productInventoryDetailsComponent = readFileSync(join(repoRoot, "components", "ProductInventoryDetails.tsx"), "utf8");
const missingCatalogPage = readFileSync(join(repoRoot, "app", "owner", "catalog", "missing", "page.tsx"), "utf8");
const missingListingsWorkspaceComponent = readFileSync(join(repoRoot, "components", "MissingListingsWorkspace.tsx"), "utf8");
...
const dashboardModel = readFileSync(join(repoRoot, "lib", "dashboard.ts"), "utf8");
...
const appNavigationModel = readFileSync(join(repoRoot, "lib", "app-navigation.ts"), "utf8");
...
assert.match(dashboardPage, /getDashboardOverview/, "Dashboard route exists and uses the bounded dashboard overview model");
assert.match(dashboardPage, /Selected seller account/, "Dashboard shows selected company and account context");
assert.match(dashboardPage, /account\.marketplace/, "Dashboard shows selected marketplace context");
assert.match(dashboardPage, /ActionGroup[\s\S]*overview\.actions\.operations[\s\S]*overview\.actions\.imports/, "Dashboard renders account-scoped actions from the overview model");
assert.match(dashboardModel, /href: "\/work"[\s\S]*href: "\/work\/scan"[\s\S]*href: "\/owner\/product-inventory\/refresh"[\s\S]*href: "\/owner\/imports"/, "Dashboard model exposes the modern Work Hub and import destinations");
assert.match(productInventoryPage, /InventoryCard[\s\S]*searchProductInventory[\s\S]*Selected seller account only/, "Product Inventory delegates bounded account-scoped results to its compact card component");
assert.match(productInventoryCard, /marketplaceIdentities[\s\S]*StatusBadge value=\{listing\.listingStatus[\s\S]*Details/, "Product Inventory cards expose lifecycle state, marketplace identities, and a detail action");
assert.match(productDetailPage, /requireUser\(\['OWNER'\]\)[\s\S]*requireAccount\(user\)[\s\S]*loadProductDetail[\s\S]*ProductInventoryDetails/, "Product detail adapter preserves owner/account authorization and delegates to the bounded detail workspace");
assert.match(productInventoryDetailsComponent, /StatusBadge[\s\S]*Seller SKU[\s\S]*Default processing[\s\S]*Manual protection/, "Product detail establishes product identity before configuration controls");
assert.match(productInventoryDetailsComponent, /ProfessionalProcessRuleEditor[\s\S]*saveCatalogFieldLocksAction/, "Product detail preserves authoritative process-rule and manual-protection actions");
...
assert.match(missingCatalogPage, /MissingListingsWorkspace[\s\S]*searchParams=\{searchParams\}/, "Missing Listings route delegates its query state to the bounded workspace");
assert.match(missingListingsWorkspaceComponent, /requireUser\(\["OWNER"\]\)[\s\S]*requireAccount\(user\)[\s\S]*loadMissingListings/, "Missing Listings preserves owner/account authorization and account-scoped reads");
assert.match(missingListingsWorkspaceComponent, /Work remains held until an exact account-scoped listing is selected or created[\s\S]*unresolved row[\s\S]*Orders \/[\s\S]*Consignments/, "Missing Listings exposes scalable source counts and operational hold language");
...
assert.match(appNavigationModel, /href: "\/dashboard"[\s\S]*label: "Dashboard"[\s\S]*section: "OVERVIEW"/, "Owner navigation uses /dashboard as the dashboard link");
assert.match(appNavigationModel, /hasWorkPermission\(user, "canPick"\)[\s\S]*href: "\/work\/pick"/, "Picker navigation opens the canonical projection queue");
assert.match(appNavigationModel, /hasWorkPermission\(user, "canPack"\)[\s\S]*href: "\/work\/pack"/, "Packing navigation derives from the authoritative work permission");
assert.match(appShell, /function linksForUser[\s\S]*navigationForUser\(user\)/, "The Server Component delegates to the centralized permission-aware navigation model");
assert.match(appNavigationModel, /if \(user\.role === "OWNER"\) return \[\.\.\.ownerNavigation\][\s\S]*hasWorkPermission/, "Owner management links remain separate while workers may combine operational permissions");
...
assert.match(appNavComponent, /hidden[\s\S]*xl:flex[\s\S]*data-desktop-sidebar/, "Desktop sidebar is hidden until the evidence-backed 1280px breakpoint");
assert.match(appNavigationModel, /\/owner\/accounts/, "Owner navigation includes account management");
...
assert.match(appShell, /<MobileDrawer[\s\S]*links=\{links\}/, "Owner mobile navigation uses the compact shared drawer");
