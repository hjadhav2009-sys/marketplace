...
assert.match(appNavigationModel, /hasWorkPermission\(user, "canPick"\)[\s\S]*href: "\/work\/pick\?source=ORDER"/, "Picker navigation opens the source-aware projection queue");
assert.match(appNavigationModel, /hasWorkPermission\(user, "canPack"\)[\s\S]*href: "\/packing"/, "Packing navigation derives from permission with legacy role fallback");
