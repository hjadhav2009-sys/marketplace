...
const problemsWorkspace=read("src/lib/workflow/problems-workspace.ts");
...
assert.match(problems,/ProfessionalProblemsPage/);
assert.match(problemsWorkspace,/userCanViewAllConsignmentWork/);
assert.match(problemsWorkspace,/const canManage = userCanManageConsignmentTasks/);
assert.doesNotMatch(problemsWorkspace,/const manager=.*canViewAllWork/);
assert.match(problemsWorkspace,/canManage \? client\.user\.findMany/);
